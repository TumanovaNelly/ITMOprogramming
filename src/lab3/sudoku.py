...
def get_block(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    return [grid[row][col] for row in range(start_row, start_row + BLOCK_SIZE) for col in range(start_col, start_col + BLOCK_SIZE)]
...
def find_possible_values(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.Set[str]:
    ...
    True
    """
    return {str(num) for num in range(1, 10)}.difference(get_row(grid, pos)).difference(get_col(grid, pos)).difference(get_block(grid, pos))
...
def check_solution(solution: tp.List[tp.List[str]]) -> bool:
    ...
    for row in range(len(solution)):
        for col in range(len(solution[row])):
            if not (set_num == set(get_row(grid, (row, col))) == set(get_col(grid, (row, col))) == set(get_block(grid, (row, col)))):
                return False 
